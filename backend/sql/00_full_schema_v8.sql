-- SMART_HOME V8 - FULL SCHEMA
-- Dùng khi Supabase mới hoàn toàn hoặc bạn muốn làm lại từ đầu.
-- Luồng chính: user đăng nhập -> nhập 1 mã nhà -> ai nhập trước là chủ nhà.
...
create table if not exists public.esp32_devices (
  ...
  gateway_role text not null default 'control'
...
-- App hiện dùng backend service role cho ghi dữ liệu. Policy này chủ yếu cho đọc an toàn nếu client truy cập Supabase trực tiếp.
